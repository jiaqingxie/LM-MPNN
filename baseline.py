...
def test(loader, model, target, std, args):
    model.eval()
    error = 0
    if args.task == "reg":
        for data in loader:
            data = data.to(args.device)
            error += (model(data.input_ids, data.attention_mask)[0] * std - data.y[:, target] * std).abs().sum().item()  # MAE for regression
        return error / len(loader.dataset)
    else:
        acc = 0
        for data in loader:
            data = data.to(args.device)
            acc += (model(data.input_ids, data.attention_mask)[0].argmax(dim=1) == data.y[:, target]).sum()  #  acc for classification
...
if __name__ == "__main__":
    ...
    # model
    property_model = ChemBERTaForPropertyPrediction(pretrain_chemberta, args.out_dim, args.task).to(args.device)
    optimizer = torch.optim.Adam(property_model.parameters(), lr=args.lr)
    ...
    for epoch in range(args.epochs):
        ...
        valid_metric = test(valid_loader, property_model, args.target, target_std, args)
        scheduler.step(valid_metric)
        test_metric = test(test_loader, property_model, args.target, target_std, args)

        if args.task == "reg":
            print(f'Epoch: {epoch:03d}, LR: {lr:7f}, Loss: {train_loss:.7f}, '
                f'Val MAE: {valid_metric:.7f}, Test MAE: {test_metric:.7f}')
        elif args.task == "clf":
            print(f'Epoch: {epoch:03d}, LR: {lr:7f}, Loss: {train_loss:.4f}, '
                f'Val ACC: {valid_metric:.4f}, Test ACC: {test_metric:.4f}')