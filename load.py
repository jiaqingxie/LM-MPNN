...
class LM_QM9(InMemoryDataset):
    ...
    def process(self):
        ...
        for idx, data in enumerate(qm9):
            data.input_ids = tokenized_features[idx].unsqueeze(0)
            data_list.append(data)
            is_padding = (data.input_ids == 1)
...
class LM_MoleculeNet(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pretrain_model = "ChemBERTa-zinc-base-v1", padding = True, name = "ESOL"):
        self.pretrain_model = pretrain_model
        self.padding = padding
        self.name = name
    ...
    def process(self):
        ...
        smiles_list = [data.smiles for data in mol]
        tokenized_features = self.pretrain(smiles_list, self.pretrain_model)

        data_list = []
        for idx, data in enumerate(mol):
