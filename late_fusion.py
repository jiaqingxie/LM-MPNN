...
if __name__ == "__main__":
    ...
    # late fusion model
    late_model = LateFusionModel(pretrain_chemberta,
                                 dataset.num_features,
                                 gnn_hidden_dim,
                                 graph_embedding_dim).to(device)
    late_optimizer = torch.optim.Adam(late_model.parameters(), lr=1e-3)
    ...
    for epoch in range(num_epochs):
        ...
        valid_error = test(valid_loader, late_model, target, target_std)
        late_scheduler.step(valid_error)
        test_error = test(test_loader, late_model, target, target_std)
